...
final class MetaboxModule implements IntegrationModule {
	...
	public function register( CapabilityRegistry $registry ): void {
		...
		// ONE VALUE PROJECTION SHARED BY THE INPUT, THE FORWARD PATH AND THE RECOVERY
		// PATH. The input asks whether a caller's value would survive it; the forward
		// path plans and verifies against it; the recovery path consults it only to ask
		// whether a raw value could be recorded faithfully. Two objects holding that
		// rule would eventually hold two versions of it, and the write would then be
		// checked against a spelling its own rollback does not use.
