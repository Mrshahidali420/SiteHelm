...
The four `docs/research/` documents named above no longer exist. They were the
working notes behind the Phase 1 gates and were removed when the repository was
published; their conclusions are what the requirements matrix and the foundation
contract record. The lines are left as written because this file is a record of
what the gates checked at the time, not a set of links to follow now.
...
- **Runtime `outputSchema` validation is deferred** per recorded interpretation I6.
  ...
  per-operation conformance test for each of the twelve registered operations
  (five at Phase 3a, plus `content-list` and `taxonomy-list` from part 1,
  `content-featured-media-set` and `content-status-set` from part 2, and
  `content-meta-update`, `content-terms-assign` and `content-trash` from part 3,
  shipped 2026-08-01), covering both branches of the write union. Validation at the dispatcher's
