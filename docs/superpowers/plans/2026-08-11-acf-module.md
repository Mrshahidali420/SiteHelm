...
- **The input-test split is already done** — Task 4's fix round performed it when the file crossed 800 lines. `AcfFieldUpdateInputTest` (560) keeps the general cases, `AcfFieldUpdateLayoutsTest` (213) holds the flexible-content cases, and the shared builders live in the `AcfFieldUpdateRequests` trait (160). Task 5 adds nothing to these; it only needs to know the seam exists, and that a builder edited in the trait changes meaning for both classes.
