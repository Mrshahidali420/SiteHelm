...
- Extracting `ContentRollbackApply`, now **790 lines** after REQ-0015's and REQ-0016's meta and term loops landed in it on top of REQ-0017's media loops. Still out of scope; only the figure moved. It is **10 lines** from the 800-line ceiling, so the next change to this file is likely to force the extraction. This is the largest file under `src/` and the one to watch.
