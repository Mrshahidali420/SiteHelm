...
**Recorded 2026-08-01, once all five Phase 3b writes shipped.** The predictions above are left exactly as written — they were the argument for the contract and they were right. What can now be added is which of them a real operation exercises:

- **REQ-0019, the trash slug rename: live and pinned**, by `ContentTrashTest::test_the_plan_promises_the_trash_status_and_the_renamed_slug` (the `__trashed` suffix is modelled in the promise, so it verifies rather than warning) and `::test_a_slug_wordpress_uniquified_on_collision_is_an_adjustment_not_a_failure` (the collision suffix the operation cannot model, classified as an adjustment). The warning wording fix in Decision 4 is what makes the second of those readable: it names the field and no longer accuses a plugin.
- **REQ-0015, metadata unslashing: live and pinned**, by `ContentMetaUpdateTest::test_apply_change_slashes_the_value_on_the_way_in`.
- **REQ-0018, `publish`→`future`: not exercised, and now not expected to be.** `content-status-set` deliberately omits `future` from its settable enum, so the operation cannot ask for the transition that produces it. The row above stands as a description of what WordPress does, not of a case this engine reaches.
- **REQ-0017, a dropped featured media id: not exercised, by design.** Interpretation I7 requires the reference to be validated while planning, so an unresolvable attachment id is refused with `invalid_input` and never reaches verification. That is the row's prediction being *prevented* rather than handled.

**One correction to the table itself:** REQ-0016 normalising cleanly is true only for taxonomies that do not store term order. A taxonomy registered `sort => true` makes `wp_set_object_terms()` rewrite `term_order` over the tt_ids it accumulated in the order the caller passed them, filtered against a fresh read of what is now attached — near-literally the passed order, not literally it — while the read projection sorts numerically and cannot see that order — so the set round-trips while the sequence does not. Both `content-terms-assign` and `content-rollback-apply` refuse at plan time with `rollback_unavailable` rather than verifying a write that silently flattened it, but **at different scopes, and the difference matters**. `content-terms-assign` refuses on any such taxonomy the item carries. `content-rollback-apply` refuses only when the restoration would actually write terms — a column-only rollback proceeds, because refusing it would block the recovery path over a taxonomy it never touches — and pairs that with omitting the `terms` key from its own capture, so the narrower refusal cannot hand out a rollback reference its own reverse would reject.
