...
**One correction to the table itself:** REQ-0016 normalising cleanly is true only for taxonomies that do not store term order. A taxonomy registered `sort => true` makes `wp_set_object_terms()` rewrite `term_order` over the tt_ids it accumulated in the order the caller passed them, filtered against a fresh read of what is now attached — near-literally the passed order, not literally it — while the read projection sorts numerically and cannot see that order — so the set round-trips while the sequence does not. Both `content-terms-assign` and `content-rollback-apply` refuse such an item at plan time with `rollback_unavailable` rather than verifying a write that silently flattened it.
