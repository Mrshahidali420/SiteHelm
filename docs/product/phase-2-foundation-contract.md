...
| `verification` | Post-write verification status: `verified` when the engine re-read WordPress state and confirmed it matches the approved plan payload; `not-applicable` for reads. A write whose re-read diverges from the plan never returns a result; it returns `verification_failed`. |
