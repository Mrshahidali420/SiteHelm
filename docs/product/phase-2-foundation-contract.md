...
**Amended 2026-09-06:** The `retryable` field is defined narrowly: it says whether sending THIS REQUEST AGAIN UNCHANGED can succeed later, and nothing else. `invalid_input` and `stale_plan` therefore report it as false. Both are cleared by sending a DIFFERENT request — corrected arguments, or a fresh plan token — which the Retryability column below has always said in words while the flag said the opposite. A real session returned `invalid_input` carrying `retryable: true` beside the message “identical input always fails identically”, and a client reading the flag rather than the prose retries a call that can never work. The “there is something you can do about this” meaning the old value reached for belongs to `remediation`, where it already lives. Recorded as interpretation I9.
**Amended 2026-09-09:** The `isDestructive` field is renamed `losesStateWithoutSnapshot`, and it and `isIdempotent` are both published in the operation catalogue, where neither appeared before. The meaning, the allowed values and the cross-field rules are untouched — only the name and the audience change. The old name read as a judgement about how bad the operation was, and on that reading it was wrong on the two operations where a caller most needs the answer: `media-delete` removes a file and every resized copy and can never be undone, and declared it false; `content-trash` is reversed with two clicks, and declared it true. Recorded as interpretation I10.
...
| `losesStateWithoutSnapshot` | True when the operation removes or wholesale replaces existing user-visible state such that data would be lost without a snapshot (moving content to trash, removing an element, saving a full builder document). Such operations must declare `previewPolicy`, `snapshotPolicy`, and `rollbackPolicy` all `required`. Named `isDestructive` until 2026-09-09; see interpretation I10. | `true`, `false`. |
...
- `mode` = `read` forces `isReadOnly` = `true`, `losesStateWithoutSnapshot` = `false`, and all three policies `not-applicable`.
- `losesStateWithoutSnapshot` = `true` forces `previewPolicy`, `snapshotPolicy`, and `rollbackPolicy` all `required`.
...
| `retryable` | Whether sending this request again unchanged can succeed later, per the table below. It is false whenever the way forward is a different request — corrected arguments, or a fresh plan — even though those are recoverable; what to do instead is named in `remediation`. |
...
| `invalid_input` | The request payload violated the strict input schema: missing property, wrong type, constraint violation, or an unknown property. Also returned for uploads that fail MIME or size validation. | Not retryable: identical input always fails identically. Correcting the input is a different request, and `remediation` names what to correct. |
...
| `stale_plan` | The plan token is expired, already used, unknown, or bound to a different user, site, operation, target, or payload. State remains untouched. | Not retryable: the same token stays stale. Generating a fresh preview and approving that token is a different request. |
