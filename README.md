...
| **Surface** | 200+ loosely specified tools | 56 operations behind 11 dispatchers, each with a strict JSON Schema |
...
56 operations across seven modules, reached through 11 MCP tools (dispatchers). Call any dispatcher with no `operation` argument to get its catalogue — agents discover the surface at runtime instead of memorising it.
