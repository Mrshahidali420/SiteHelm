...
[![Operations](https://img.shields.io/badge/operations-98-blueviolet.svg)](docs/OPERATIONS.md)
...
SiteHelm is a WordPress plugin that exposes your site to AI agents over the [Model Context Protocol](https://modelcontextprotocol.io/). Claude, Claude Code, Cursor, VS Code, or any other MCP client can read your content, edit Elementor pages, manage media and menus, write ACF and Meta Box fields, and edit SEO metadata in Yoast or Rank Math — through **98 typed operations**, every one of them capability-checked, previewed before it runs, snapshotted before it changes anything, and verified afterwards by reading the site back.
...
| **Surface** | 200+ loosely specified tools | 98 operations behind 11 dispatchers, each with a strict JSON Schema |
...
98 operations across nine modules, reached through 11 MCP tools (dispatchers). Call any dispatcher with no `operation` argument to get its catalogue — agents discover the surface at runtime instead of memorising it.
