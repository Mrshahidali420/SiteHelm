...
| **Surface** | 200+ loosely specified tools | 51 operations behind 11 dispatchers, each with a strict JSON Schema |
...
51 operations across seven modules, reached through 11 MCP tools (dispatchers). Call any dispatcher with no `operation` argument to get its catalogue — agents discover the surface at runtime instead of memorising it.
