<?php
/**
 * The committed baseline of every menus operation's declared schemas.
 *
 * @package SiteHelm
 */
...
/**
 * Pins every declared byte of every menus operation's input and output schema
 ...
 * say are asserted by name in MenusDefinitionInvariantsTest, which reads no
 * fixture.
 */
final class MenusDefinitionBaselineTest extends TestCase {
	...
	public static function baselinePath(): string {
		return dirname( __DIR__, 3 ) . '/Fixtures/menus-operation-definitions.json';
	}
	...
	public function test_every_declared_schema_matches_the_committed_baseline(): void {
		...
		$this->assertStringEqualsFile(
			...
			'A declared input or output schema, an operation id, the registration order, or the operation count has moved off tests/Fixtures/menus-operation-definitions.json. The diff below names the changed line. If the change is intended, update the fixture; if it is not, restore the declaration.'
		);
	}
}
