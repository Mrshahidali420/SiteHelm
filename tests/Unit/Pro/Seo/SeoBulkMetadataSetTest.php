<?php
/**
 * content-seo-bulk-set: one licence gate, every post checked before anything is planned.
 *
 * @package SiteHelmPro
 */
...
use SiteHelm\Pro\Seo\SeoBulkMetadataSet;
...
final class SeoBulkMetadataSetTest extends TestCase {
	...
	private function operation(): SeoBulkMetadataSet {
		return new SeoBulkMetadataSet( $this->licence(), new SeoPresence() );
	}
	...
	public function test_an_unlicensed_site_is_refused_before_any_post_is_looked_at(): void {
		$this->installYoast();

		try {
			$this->operation()->resolveTarget( [ 'ids' => [ 42 ] ], $this->context() );
