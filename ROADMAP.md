...
| REQ-0117 | Core | **Install from a zip already in the media library** — Pro, one attachment-id argument, the package read and refused before a byte moves, an installed package overwritten with its old files copied aside first so the change can be rolled back; overwriting the live theme is the normal case, because that is how a child theme is iterated | should |
