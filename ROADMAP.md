...
| REQ-0085 | Core | **Plugins and themes** — list what is installed and what has an update; activate, deactivate and update a plugin or theme (reversible by construction: the previous state is the rollback); installing from WordPress.org only, never an arbitrary zip | Free: list and update status · Pro: act | could — **Free half shipped 2026-08-30** (SiteHelm 0.9.0): `system-plugin-list`, `system-theme-list` — the whole inventory with active state, network-activation, and the update column exactly as WordPress's own last check left it, timestamped and never asking wordpress.org from a read. 101 operations across 11 dispatchers. **Pro half shipped 2026-08-30** (SiteHelm Pro 0.7.0): `plugin-activate`, `plugin-deactivate` and `theme-switch` snapshot the state they replace and restore it through every guard they applied forwards; `plugin-update`, `theme-update`, `plugin-install` and `theme-install` refuse a rollback outright rather than pretend to one, because WordPress has no clean downgrade. Installing takes a WordPress.org slug and nothing else — no property in either schema accepts a web address, a zip or a file path — the only address fetched is the download link wordpress.org itself answers with, and what lands is stored deactivated. `DISALLOW_FILE_MODS` / `DISALLOW_FILE_EDIT` stop the four file writes by name and leave the three option flips alone. Complete, and extended by REQ-0117, which adds a second source under the same two capabilities: a zip the operator has already put in the site's own media library |
...
| REQ-0121 | Core | **The gaps a real build found** — a site was built end to end through SiteHelm alone, and everything it could not do was written down: custom fields that could be read but not written, meta values that arrived as text whatever their type, private post types that no listing showed, paging and status friction on the content list, no way to put pages in a chosen order, no way to set the site icon or the logo, and no way to make the first menu | Free | must — **Shipped 2026-09-05**: `content-meta-set` reaches the allowlist and keeps a value's type; `content-list` shows private and any post types and pages honestly; `content-create` and `content-update` take `menuOrder`; `site-settings-set` takes `siteIcon` and `siteLogo`; `menu-create` makes the first menu, so the menu dispatcher is reachable on a site that has none. 105 operations across 11 dispatchers. Complete |
| REQ-0117 | Core | **Install from a zip you already uploaded** — a second source for the two install operations: a zip that is already in this site’s own media library, so a child theme or a plugin that is not in the directory can reach the site without SiteHelm ever being handed an address | Pro | should — **Shipped 2026-09-05**: `plugin-install-upload` and `theme-install-upload`. The only argument is the attachment id, so there is still no property anywhere in the plugin that accepts a web address or a file path. The preview opens the package and shows what it would replace, version against version; installing over something already there copies the old files aside first, so the change can be rolled back; and overwriting the live theme is allowed and leaves it live, because that is the case this exists for. The free plugin gains only the filter the add-on uses to let a zip into the media library at all, and it cannot widen past what the site itself refuses. Complete |
...
| REQ-0119 | Core | **A file on your own computer can reach the site** — a package is too large to travel as an operation argument, so the permission to upload and the bytes themselves take different roads | Free | must — **Shipped 2026-09-05**: `media-upload-ticket` returns a secret, an upload URL and an expiry; the client posts the raw file to that URL with the secret in a header, and the bytes never pass through the agent's context at all. A ticket is bound to one site, one operator, one filename and one exact byte length, optionally to one sha256, lasts ten minutes, and is spent once by a conditional update that reports how many rows it changed, so two requests presenting the same ticket produce one upload. The secret is stored only as a digest and is deliberately left out of the operation's promised fields, so it reaches the caller and stops there instead of being copied into the audit row. Redemption re-checks that writes are not paused, that the upload switch is on and that the operator still holds `upload_files`, and the arriving file is judged by its content exactly as `media-upload` judges one. Together with REQ-0117 this is what lets a theme or plugin zip sitting on a laptop reach a live site. 108 operations across 11 dispatchers. Complete |
| REQ-0120 | Core | **Why a page looks wrong** - report the CSS rules written for a selector on a page, which of them apply at a given viewport width, and which one wins, so a layout fix can be checked without a browser | Free | should - **Shipped 2026-09-05**: `content-style-check`. It is a stylesheet reader and says so: no layout, no scripts, no inheritance, no `var()`. A rule is reported when the subject of its selector carries what was asked for, so `.site-header .menu-toggle:hover` answers a question about `.menu-toggle` and `.menu-toggle .icon` does not. Width conditions are evaluated at the requested viewport in `px`, `em` and `rem`, in both the min/max spelling and the range form; a condition it cannot judge is reported as unjudged and held out of the cascade rather than guessed. Only the site's own stylesheets are fetched, through the same guard `content-rendered-get` uses; an off-site sheet is listed and left unread. 109 operations across 11 dispatchers |
| REQ-0122 | Core | **What the second build found** — the same site was taken further and the next round of gaps written down: a read that hid the custom fields actually on a post, three errors that told the operator the wrong thing, an update check that could not see a release that had already shipped, and no way to take a menu item back out | Free | must — **Merged, due in the next release**: `content-get` reports the fields a theme or plugin registered as well as the ones SiteHelm may write; a missing target reads as a missing target rather than a permissions problem; `retryable` means only what it says; arguments nested one level too deep are named instead of answered with the whole catalogue; Check again clears SiteHelm’s own note as well as WordPress’s, and a refusal from GitHub’s rate limiter rests only until the limit lifts; and `menu-item-delete` removes an item, refusing while it still has children because restoring one yields a new id. 111 operations across 11 dispatchers |
| REQ-0123 | Core | **The whole operation surface, in a form an agent can keep** — a dispatcher's catalogue answers what is on that one tool, and nobody reads eleven of them side by side, so a client choosing between operations had no single document naming what each one is, how risky it is, and whether it can be undone | Free | should — **Shipped 2026-09-06**: `system-catalog-export` returns every operation this site publishes, grouped by subject, as Markdown to save and grep or JSON to render, filtered by module on request. The same document is published as the `sitehelm://catalog` MCP resource, so a client that reads resources gets it without a call. Each row carries a `catalogVersion` stamp — a digest of the rows themselves, nothing from the request — so a saved copy can tell it is stale by comparing that stamp against what `system-connection` reports next time, and re-export only when it has moved. 112 operations across 11 dispatchers |
| REQ-0124 | Core | **Delete a media item** — every other media write could add or change a library item and none could remove one, so a build that uploaded the wrong file had to finish the job in wp-admin | Free | should — **Shipped 2026-09-07**: `media-delete` removes one library item and the files behind it. It is the one media operation with no way back, so it is declared that way: preview required, no snapshot and no rollback, and the plan names the file that goes, how many resized copies go with it, and which content items lose their featured image. It always deletes rather than sometimes leaving a trashed copy, because an operator approving the plan cannot see how the site is configured. 113 operations across 11 dispatchers |
| REQ-0125 | Core | **Repair a site whose addresses stopped matching its content** — a page that plainly exists answers "not found" after a plugin, theme or permalink change, and the only fix was to open wp-admin and re-save the permalinks screen | Free | should — **Shipped 2026-09-07**: `site-rewrite-flush` throws the stored address rules away so WordPress works them out again on the next visit, and the preview says that the first visit after the change is the slow one. The rules are deleted rather than rebuilt on the spot, because rebuilding inside the request that asks for it saves the rules the request started with, which are the stale ones. The same correction lands on `site-settings-set`, which had been rebuilding after a permalink change and so had been storing rules for the structure it had just moved away from. 114 operations across 11 dispatchers |
| REQ-0126 | Integrations | **A sitemap that no longer matches the site** — Yoast and Rank Math build the XML sitemap once and keep serving that copy until something tells them to build it again, which they cannot do when the change came from an import, a restore, or a post type another plugin registers; the sitemap then lists pages that are gone and misses pages that exist, and nothing in the plugin's settings looks wrong | Pro | should — **Shipped 2026-09-07**: `seo-sitemap-get` says which plugin is in charge, whether the sitemap is switched on, its address, how many built copies are stored, and what the site actually answers when that address is asked for. `seo-sitemap-flush` throws those copies away so the next request builds a fresh one, and the preview says that request is the slow one. The clearing goes through the SEO plugin's own routine, so a plugin that keeps its sitemap as files on disk has the files removed and not merely the register of them; a plugin without that routine is refused rather than reported as cleared. Nothing is kept to put back, because what was cleared was the out-of-date copy |
| REQ-0127 | Core | **Derived data left stale by a change WordPress never saw** — an SEO score and sitemap entry, a page builder's compiled CSS, a caching plugin's stored copy and a translation index are all worked out when a page is saved and refreshed by nothing else, so content that arrived from a backup, an import or a row edited in the database leaves every one of them wrong while the page itself is right | Free | should — **Shipped 2026-09-07**: `content-resave` stores the page again with every field exactly as it is, so everything watching for a save recalculates. The whole row is handed back escaped rather than the identifier alone, because passing the identifier makes WordPress read the stored row and unescape text that was never escaped, which eats every backslash in the content on a save that reports success. The promise covers the title, slug, content, excerpt, type and status and deliberately leaves out the item's custom fields, its terms and its modified time — those are where the plugins being woken up write their answers, and moving the timestamp is the point. 115 operations across 11 dispatchers |
| REQ-0128 | Gateway | **An operation could not say what it costs beyond the change it names** — saving a page runs whatever code the plugins on that site have hooked to a save, clearing permalinks throws away work the site had already done, and deleting a menu item or an image leaves other parts of the site still pointing at it, none of which is a risk level or a destructive flag, so an agent could only learn it by doing it | Free | should — **Shipped 2026-09-08**: every operation carries a list of consequences drawn from a closed set of five, each with a sentence a person can read, published by both the live catalogue and the saved export. A consequence may only be declared when it is true every time the operation runs, since a definition is built once with no arguments in hand; anything that depends on what was passed stays a preview warning. A read may not declare one at all. 115 operations across 11 dispatchers |

## Shipped — v0.13.0

109 operations across 11 dispatchers. REQ-0120: why a page looks wrong, answered from the
site’s own stylesheets instead of a browser. REQ-0118: a theme’s files listed and read
before anything replaces them. REQ-0119: a file on your own computer reaches the site through
a one-use ticket, so the bytes never travel as an operation argument. REQ-0117: a plugin or
theme installed from a zip already sitting in the media library, in Pro. REQ-0121: the gaps a
real build found — the first menu, the site icon and logo, private post types, a chosen
order for pages, and custom fields that keep their type. Pro also gained snippet storage in
WPCode and Code Snippets, and plugin and theme deletion.

## Shipped — v0.12.0

REQ-0108: a page’s front end fetched over loopback, so a client sees what a visitor is
served rather than the row it wrote. Buying and activating Pro moved onto a screen SiteHelm
owns. An Elementor text edit keeps the widget editable and the links inside it, a large
document comes back in a shape the client can actually receive, and a section can be asked for
full bleed instead of being told what padding to remove.

## Shipped — v0.11.0

A client can sign in to the site itself instead of being handed a password, and the connection
is one you can see and cut off from the site rather than from the app. Home opens with five
steps that tick themselves off by reading the site back. A client that asks for an older MCP
revision gets the one it asked for, and two schema shapes strict clients refused to load are
fixed.

## Shipped — v0.10.0

Connecting now returns the instructions an agent needs to build a page, and
`elementor-document-get` reports what a page has earned a note about. Bulk SEO metadata and
audit fixes moved into the free plugin: batch size is not a reason to charge when the free
plugin already ships the single write each of them repeats. A page whose elements were written
without ids lost every style on it, repeater rows could not be styled on their own, ordinary
widgets could not be edited at all, and a page-layout write changed only one of the two rows
that store it — all fixed here.

## Shipped — v0.9.0

101 operations across 11 dispatchers. REQ-0085: the plugin and theme inventory, with the
update column exactly as WordPress’s own last check left it. Acting on it is Pro.

## Shipped — v0.8.0

Updates straight from GitHub, so an installed copy learns about a release without a directory
listing. The console says when Pro would have helped, and the WordPress.org listing files land.

## Shipped — v0.7.0

99 operations across 11 dispatchers. REQ-0062 site settings, REQ-0083 five more SEO plugins
through the one vocabulary, REQ-0084 forms, REQ-0057 the WooCommerce groundwork, REQ-0092
site-wide content search, and the Elementor band — REQ-0101 global classes, REQ-0102 the
template library, REQ-0103 page-level editing, REQ-0104 whole-document writes, REQ-0105 SVG
upload — with the two safety requirements that had to come first, REQ-0106 and REQ-0107.
